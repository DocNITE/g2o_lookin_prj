...
local ChatPlayerLine = class extends ChatLine
{
	constructor(pid, r, g, b, text)
	{
		...
		_nickname = Draw(0, 0, getCharById(pid).name + ": ")
...
addEventHandler("onKey", function(key)
{
	...
	{
		//doc edit:
		if (isKeyPressed(KEY_LMENU) && isKeyToggled(KEY_LSHIFT)) {
			if (getKeyLayout() == KEY_LAYOUT_EN)
				setKeyLayout(KEY_LAYOUT_RU);
			else
