local utf8_windows1252_map =
{
    ...
    ["\xc3\x8c"] = "\xcc", // I with acute
    ["\xc3\x8d"] = "\xcd", // I with grave
    ...
    ["\xc3\xac"] = "\xec", // i with acute
    ["\xc3\xad"] = "\xed", // i with grave
...
local function utf8_to_windows1252(text)
{
    local result = ""
    ...
    for (local i = 0, charSize = 0, textLen = text.len(); i != textLen; i += charSize)
    {
        ...
        if (charSize == 1)
            result += charBytes
        else if (charBytes in utf8_windows1252_map)
            result += utf8_windows1252_map[charBytes]
        else
            result += "?"
    }
