local windows1258_utf8_map =
{
    ...
    [0xdd] = "\xc6\xaf", // U with horn
    [0xde] = "\xe2\x97\x8c\xcc\x83", // tilde above
    [0xdf] = "\xc3\x9f", // eszett

    [0xe0] = "\xc3\xa0", // a with grave
    [0xe1] = "\xc3\xa1", // a with acute
    [0xe2] = "\xc3\xa2", // a with circumflex
    [0xe3] = "\xc4\x83", // a with breve
    ...
    [0xfd] = "\xc6\xb0", // u with horn
    [0xfe] = "\xe2\x82\ab", // dong sign
    [0xff] = "\xc3\xbf", // y with diaeresis
}

local function windows1258_to_utf8(text)
{
    local result = ""
    ...
    foreach (idx, charId in text)
    {
        ...
        if (charId in windows1258_utf8_map)
            result += windows1258_utf8_map[charId]
        else
