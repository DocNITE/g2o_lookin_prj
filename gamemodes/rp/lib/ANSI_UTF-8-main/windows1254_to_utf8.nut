local windows1254_utf8_map =
{
    ...
    [0xdd] = "\xc4\xb0", // I with dot above
    [0xde] = "\xc5\x9e", // S-cedilla
    [0xdf] = "\xc3\x9f", // eszett
    ...
    [0xfd] = "\xc4\xb1", // dotless i
    [0xfe] = "\xc5\x9f", // s-cedilla
    [0xff] = "\xc3\xbf", // y with diaeresis
}

local function windows1254_to_utf8(text)
{
    local result = ""
    ...
    foreach (idx, charId in text)
    {
        ...
        if (charId in windows1254_utf8_map)
            result += windows1254_utf8_map[charId]
        else
