local utf8_windows1254_map =
{
    ...
    ["\xc4\xb1"] = "\xfd", // dotless i
    ["\xc5\x9f"] = "\xfe", // s-cedilla
    ["\xc3\xbf"] = "\xff", // y with diaeresis
}

local function utf8_to_windows1254(text)
{
    local result = ""
    ...
    for (local i = 0, charSize = 0, textLen = text.len(); i != textLen; i += charSize)
    {
        ...
        if (charSize == 1)
            result += charBytes
        else if (charBytes in utf8_windows1254_map)
            result += utf8_windows1254_map[charBytes]
        else
            result += "?"
    }
